...
0. **Pick the output mode first.** The default is **Canvas** — draw.io's infinite canvas with **no page-size limit** (`page="0"`), for screen/editing, where you let the diagram grow as large as it needs and never distill to fit. Other modes target a fixed medium (full-page Letter, a two-column `figure*`, a single column, a 16:9 slide) and constrain size, font floor, and density. **Ask the user which mode** — explicitly ask "are you in canvas mode?" if unsure. See [Output modes](#output-modes-pagination).
...
**Before generating, ask which output mode the diagram targets** — it sets the canvas size, font floor, and how much detail can stay legible. See [resources/output-modes.md](resources/output-modes.md) for full specs.

| Mode | Size (draw.io units) | Body / title floor | Density |
|---|---|---|---|
| **Canvas (default)** | infinite — `page="0"`, no limit | 14–16 / 22+ | as much as needed — **never distill to fit** |
| **1 · Full page (Letter)** | `850×1100` portrait or `1100×850` landscape | 14 / 22 | medium — split across pages if it won't fit at the floor |
| **2 · Two-column figure\*** | wide ~`1500×650` | 16 / 24 | medium-high — circled-letter labels for prose refs |
| **3 · One column** | narrow ~`750`-wide, tall | 18 / 26 | low — distill to one idea, ≤ ~10 nodes |
| **PPTX · 16:9 slide** | `1920×1080` | 22 / 32 | lowest — one high-level idea |

**Golden rule:** never shrink text below the floor to make content fit. Split pages (Mode 1) or step down in density. Legibility wins. Modes 2/3 use the lettered-badge motif as `\circled{A}` references; export SVG/PDF for LaTeX.

## Work iteratively

Diagrams are collaborative — do **not** batch-generate a multi-page or non-trivial diagram in one shot. For **each page**, ask the user **several questions first** (what it must convey, audience, must-include elements, level of detail, layout/orientation), draft it, show it, and refine before moving to the next page. Confirm the output mode (default Canvas) up front. Prefer many small, reviewed steps over one large autonomous pass.

## No overlaps — STRICT

**NON-NEGOTIABLE: nothing in a diagram may overlap.** No node may sit on top of another node; no edge may pass through a node it doesn't connect to; edge–edge crossings must be minimized. A tangled diagram is a failed diagram — legibility wins over completeness, always, in every output mode. Do not rely on draw.io's auto-router to sort this out; lay it out so it is clean by construction.

How to guarantee it:

- **Lay out in layers by depth.** One tier per column (left→right) or row (top→down). Edges run only between *adjacent* layers, through the empty channel between them — never across or over a box.
- **Compute geometry; never eyeball.** Track a y-cursor per column: each box's `y` = previous box's bottom + a fixed gap (≥ 40). Size each box's height to fit its text (≈ header + rows × line-height) so boxes never run into neighbours. Leave wide channels (≈ 150–220) between columns for edges.
- **Children sit in their parent's band.** Place a parent's children directly across from it, ordered top-to-bottom, within the parent's vertical extent. Pin the edges with `exitX=1;exitY=0.5;entryX=0;entryY=0.5` (right-center → left-center) so they fan cleanly with zero crossings.
- **Recursion is a local self-loop** on the box's own edge (`exitX`/`entryX` on the same side), not a wire flung across the canvas.

**If the graph is not a tree (a DAG — shared or repeated targets), drawing every edge WILL cross. STRICT response:**

- **Reduce it to a tree first.** Give each node ONE primary parent and draw only those edges. Show secondary / cross-cutting references (a shared type used by many parents) as **fields or a labelled note inside the box, not as arrows.** Say in the legend that cross-links are text-only.
- If a particular cross-link must be drawn, add it back **one at a time** as a clearly-distinct (dashed / curved) edge and confirm it introduces no box crossing.
- A "mixin" type attached to almost everything goes in its own labelled cluster with edges omitted and a note — never fan an arrow from every owner to it.

**Verify before showing the user:** scan for overlapping rectangles and trace each edge's corridor to confirm it never enters a non-endpoint box. If you cannot make every edge clean, drop edges (fall back to the tree) — never ship overlap.

## Visual style

Apply the house design system to every diagram unless the user requests a different style:

- **Colour & type** — IBM Carbon hues in a tiered, dark-mode-native scheme + IBM Plex Sans. See [resources/color-palette.md](resources/color-palette.md) for the palette, font settings, and ready-to-use cell styles.
- **Structure & grammar** — labelled regions, tiered families, semantic shapes, lettered badges, edge-label chips. See [resources/design-grammar.md](resources/design-grammar.md), distilled from the diagrams in `samples/` (study `samples/main-arch.drawio` first).

Four rules worth stating up front:

- **No overlaps (STRICT).** Nodes never overlap; edges never cross boxes. Lay out in layers and reduce DAGs to a tree — see [No overlaps — STRICT](#no-overlaps--strict) above. This is non-negotiable.
- **Dark-mode native.** Author colours as `light-dark(light,dark)` and set `adaptiveColors="auto"` on the `mxGraphModel`; verify the diagram in both themes.
- **Edge labels get a tinted chip, never white.** Put `labelBackgroundColor=light-dark(#f2f4f8,#262626);labelBorderColor=none;` on every labelled edge — the default white box stands out against the canvas.
- **Tiered colour, one warm accent.** Each semantic family in tint → border → dark-fill tiers; reserve the warm accent (red) for system titles, lettered badges, and callouts.

### Favour ridiculous simplicity

Let the design speak for itself. Do not wrap simple content in layers of containers.

- **A label is just a label.** For plain text, use a single `text` cell — never a box inside a box inside a group. One cell, done.
- **Do not nest boxes to create visual padding or grouping** unless the grouping is semantically meaningful (e.g. a real swimlane or a labelled subsystem). A drop shadow or fill is not a reason to add a wrapper cell.
- **One element per concept.** If a node and its label can be one cell with a `value`, make it one cell. Avoid a shape cell plus a separate overlaid text cell.
- Prefer whitespace, alignment, and color over borders and nesting. Reach for a container only when it carries meaning, not decoration.
